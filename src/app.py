...
for commodity in master.keys():
    data_for_commodity = {}

    for state in master[commodity].keys():
        for county in master[commodity][state].keys():
            df = master[commodity][state][county]

            if not df.empty and 'Irrigation Practice Code' in df.columns and \
               (any(df['Irrigation Practice Code'] == 2) or any(df['Irrigation Practice Code'] == 997)):
                if state not in data_for_commodity:
                    data_for_commodity[state] = {}
                data_for_commodity[state][county] = df.to_json()

    if data_for_commodity:
        with open(f'{commodity}.json', 'w') as file:
            json.dump(data_for_commodity, file)
'''
...
def return_figures(selected_commodity, start_year, end_year, comparison_year, yield_type, result_type):
    ...
    for commodity in commodities:
        ...
        # Load the data for the commodity from its JSON file
        with open(f'{commodity}.json', 'r') as file:
            data_for_commodity = json.load(file)

        # Loop through each state and county
        for state in data_for_commodity.keys():
            for county in data_for_commodity[state].keys():
                try:
                    # Convert the JSON string back to a DataFrame
                    df_json = data_for_commodity[state][county]
                    ...
                    # Handle any errors
                    print(f"Error processing {commodity} for {state} - {county}: {e}")
           
        for key in yield_diff.keys():
            try:
                if not yield_diff[key].empty:  # Check if it's not empty
                    temp_diff = yield_diff[key][(yield_diff[key].index[0])]
                    yield_diff[key] = temp_diff
                else:
                    yield_diff[key] = None
                    
                if not yield_percent[key].empty:  # Check if it's not empty
